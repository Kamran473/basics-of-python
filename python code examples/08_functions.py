...
# # example 01: In the simplest form, a user is asked to type a word, and the program uses .lower() to convert it entirely to lowercase before displaying it. This ensures that even if the user types in capital letters, the program treats everything uniformly.

# def letter_case():
#     user = input("Enter your lovely word:")
#     user = user.lower()
#     print("lowercas word:", user)
# letter_case()
    

# Example 02: In a slightly more advanced example, the program asks for a sentence from the user. It then converts the entire sentence to lowercase using .lower() and counts how many times a specific lowercase word (like "the") appears in it, ignoring original casing.
...
#     print("------****------")
#     print(f"Maximum number from input three numbes is  {max_numbr}.")
...
# Example 07: Write a Python function to calculate the factorial of a number (a non-negative integer). The function accepts the number as an argument.
# # method 01
# def factorial(n):
#     if n == 0 or n == 1:
#         return 1
